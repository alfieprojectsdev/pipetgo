...
## RULE 0 (MOST IMPORTANT): Follow standardized structure exactly (+$500 reward for perfect compliance)

ALL ADRs MUST use the standardized structure with NO deviations (-$1000 penalty for structure violations). Use ONLY the 5 approved relationship types (-$500 penalty for custom relationships).

The WORST mistake is using non-standard relationship types (-$500 penalty). The SECOND worst mistake is omitting the Revision Log (-$300 penalty). The THIRD worst mistake is putting implementation details in main body instead of Appendices (-$200 penalty).
...
**WHEN IN DOUBT:**
- Use "Depends on" relationship (safest default for unclear relationships)
- Put implementation details in Appendices (keep main body high-level)
- Add Revision Log entry for ANY change (over-document vs under-document)
- Check for existing ADR index FIRST (discover relationships before writing)
- Use EXACTLY the 5 standard relationship types (NEVER invent custom ones)
- Verify structure matches template (no standalone Date headers, etc.)

**Default: When uncertain about relationship type, use "Depends on".** It's the most broadly applicable relationship.
